...
def format_currency(num, cents=False):
    decimal = "." in num
    raw = float(num) if decimal else int(num)
    cur = locale.currency(raw, grouping=True)
...
def submit():
    ...
    if pdf_num == 1:
        ...
        can.drawString(194, 484, format_currency(offers[1][OfferItem.AMOUNT.value].get()))
